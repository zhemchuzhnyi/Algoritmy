...
if not hourly_activity.empty:
    ...
    plt.grid(True)
    plt.savefig(chart_data_hourly, format='png', bbox_inches='tight')
    plt.close()
...
title.text = "Анализ чатов Docma: Апрель 2025"
subtitle.text = "Отчет по активности и типам запросов"
...
content.text = "Основные показатели за апрель 2025:"
for idx, row in summary_df.iterrows():
    p = content.add_paragraph()
    p.text = f"{row['Метрика']}: {row['Значение']}"
    p.level = 1
    p.font.size = Pt(14)

# Слайд 3: Таблица аналитики
slide_layout = prs.slide_layouts[5]
slide = prs.slides.add_slide(slide_layout)
title = slide.shapes.title
title.text = "Таблица аналитики"
x, y, cx, cy = Inches(0.5), Inches(1.5), Inches(9), Inches(4)
table = slide.shapes.add_table(rows=len(summary_df) + 1, cols=2, left=x, top=y, width=cx, height=cy).table
table.columns[0].width = Inches(5)
table.columns[1].width = Inches(4)
table.cell(0, 0).text = "Метрика"
table.cell(0, 1).text = "Значение"
for i, row in summary_df.iterrows():
    table.cell(i + 1, 0).text = str(row['Метрика'])
    table.cell(i + 1, 1).text = str(row['Значение'])
...
# Слайд 4: Круговая диаграмма
if not query_counts.empty:
    slide_layout = prs.slide_layouts[5]
    slide = prs.slides.add_slide(slide_layout)
    title = slide.shapes.title
    title.text = "Распределение типов запросов"
    chart_data.seek(0)
    slide.shapes.add_picture(chart_data, Inches(1), Inches(1.5), Inches(8), Inches(4))

# Слайд 5: График активности
if not hourly_activity.empty:
    slide_layout = prs.slide_layouts[5]
    slide = prs.slides.add_slide(slide_layout)
    title = slide.shapes.title
    title.text = "Активность пользователей по часам"
    chart_data_hourly.seek(0)
    slide.shapes.add_picture(chart_data_hourly, Inches(1), Inches(1.5), Inches(8), Inches(4))

# Сохранение презентации
output_pptx = "docma_chats_analysis_2025-04.pptx"
prs.save(output_pptx)
print(f"Презентация сохранена как {output_pptx}")

# Сохранение в Excel (как в исходном скрипте)
...
print(f"Файл Excel сохранён как {output_file}")